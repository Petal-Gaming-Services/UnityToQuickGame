...
3.  配置**build**目录下的manifest.json文件。
    ...
    <td class="cellrowborder" valign="top" width="50%" headers="mcps1.1.4.1.3 "><p id="p126171247124310"><a name="p126171247124310"></a><a name="p126171247124310"></a>打印日志等级，分别为error及以上、warn及以上、info及以上、log及以上、debug及以上。</p>
    </td>
    </tr>
    </tbody>
    </table>
...
4.  将从Unity导出的WebGL项目存放至**快游戏打包工具**的根目录下。
5.  配置打包工具根目录下的config.json文件。
    ...
    <td class="cellrowborder" valign="top" width="40%" headers="mcps1.1.5.1.4 "><p id="p123790414425"><a name="p123790414425"></a><a name="p123790414425"></a>WebGL项目的存放路径。</p>
    </td>
    </tr>
    ...
    <td class="cellrowborder" valign="top" width="40%" headers="mcps1.1.5.1.4 "><p id="p143805417424"><a name="p143805417424"></a><a name="p143805417424"></a>证书指纹所在的绝对路径。</p>
    <div class="note" id="note18409724068"><a name="note18409724068"></a><a name="note18409724068"></a><span class="notetitle"> 说明： </span><div class="notebody"><p id="p1583403820313"><a name="p1583403820313"></a><a name="p1583403820313"></a>若此处缺省，您需要把存放certificate.pem和private.pem文件的sign文件夹放至打包工具的根目录下。</p>
    </div></div>
    </td>
    </tr>
...
6.  完成以上配置后，在打包工具的根目录下进入终端界面：

    1.  先输入如下命令行安装依赖文件。若依赖文件安装失败，可参考[处理依赖文件安装失败](FAQ.md#p19472500333)。

        ```
        npm install
        ```

    2.  再输入如下命令行构建快游戏RPK包体。成功打包的快游戏将存放在dist文件夹中。若最终RPK包体超过20MB，解决方案请参考[Unity转RPK快游戏包体过大](FAQ.md#p1667283811460)。
