...
namespace CRWASM
{
    public interface RTEnv
    {
        string USER_DATA_PATH { get; }
    }

    class RTEnvImpl : MonoBehaviour, RTEnv
    {
        #region C# 调用 JS 方法
        ...
        private static extern string RTGetUserDataPath();

#else

        private static string RTGetUserDataPath()
        {
            Debug.Log("RTGetUserDataPath not support");
            return null;
        }
        ...
        public static RTEnvImpl Create()
        {
            RTEnvImpl instance = new GameObject("RTEnv").AddComponent<RTEnvImpl>();
            DontDestroyOnLoad(instance.gameObject);
            return instance;
        }

        public static void Destroy(RTEnvImpl instance)
        {
        ...
        public string USER_DATA_PATH
        {
            get
            {
                ...
                {
                    _userDataPath = RTGetUserDataPath();
                }
